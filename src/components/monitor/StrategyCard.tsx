...
import { useStrategyDetails, useCloseStrategy, Status, RiskLevel } from "@/lib/web3/hooks";
...
export function StrategyCard({ strategyId }: StrategyCardProps) {
  const { data: strategy, isLoading, refetch } = useStrategyDetails(strategyId);
  const { closeStrategy, isConfirmingWallet, isPendingTx, isConfirmed } = useCloseStrategy();
  const isClosing = isConfirmingWallet || isPendingTx;

  // We still fetch live price to calculate health
  const { data: liveData } = useQuery<{ pool: { currentPrice: number } }>({
    queryKey: ["agent-analysis", strategy?.poolId],
    enabled: !!strategy?.poolId,
  });
  ...
  return (
    <Card className={`p-6 relative group border-card-border overflow-hidden card-hover-glow ${!isActive ? "opacity-60" : ""}`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="flex -space-x-2">
             <div className="w-8 h-8 rounded-full bg-surface-2 border-2 border-card z-10 overflow-hidden">
               {pool ? <TokenIcon src={pool.token0.icon} symbol={pool.token0.symbol} /> : "🪙"}
             </div>
             <div className="w-8 h-8 rounded-full bg-surface-2 border-2 border-card z-0 overflow-hidden">
               {pool ? <TokenIcon src={pool.token1.icon} symbol={pool.token1.symbol} /> : "🪙"}
             </div>
          </div>
          <div>
            <h3 className="font-semibold text-lg leading-tight uppercase">
              {pool ? `${pool.token0.symbol} / ${pool.token1.symbol}` : strategy.poolId}
            </h3>
            <div className="flex items-center gap-2">
              <p className="text-xs text-muted font-mono">Onchain Strategy #{strategy.id.toString()}</p>
        ...
        <Badge variant={!isActive ? "default" : inRange ? "success" : "danger"}>
          {!isActive ? "Closed" : inRange ? "Active (In Range)" : "At Risk (Out of Range)"}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div>
          ...
          <p className="font-mono text-base text-muted">{formatPrice(rangeLow)}</p>
        </div>
        <div>
          <p className="text-xs text-muted mb-1">Max Price</p>
          <p className="font-mono text-base text-muted">{formatPrice(rangeHigh)}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="h-4 w-full bg-surface-2 rounded-full relative overflow-hidden border border-card-border/50">
          <div className={`absolute top-0 bottom-0 left-0 right-0 ${isActive && inRange ? "bg-accent/20" : "bg-risk-high/10"}`} />
      ...
      {isActive && (
        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
          ...
          >
            {isClosing && <span className="w-3 h-3 border-2 border-muted border-t-transparent rounded-full animate-spin"></span>}
            {isConfirmed ? "Closed!" : isClosing ? "Closing..." : "Close Strategy"}
          </button>
        </div>
      )}
    </Card>
